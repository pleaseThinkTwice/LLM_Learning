...
$$\boxed{\text{Function Calling} = \underbrace{\text{训练}}_{\text{管语义：该调什么}} + \underbrace{\text{约束解码}}_{\text{管语法：格式对不对}} + \underbrace{\text{你的 runtime}}_{\text{管执行：真的去调}}}$$
...
**为什么？** 因为：
$$\boxed{\text{如果对 observation 算 loss，你就是在教模型生成工具的返回结果 —— 也就是教它幻觉}}$$
...
$$\boxed{\text{这个格式是 SFT 时烧死的，不是配置项}}$$
...
$$\boxed{\text{约束保证"输出是合法 JSON"，保证不了"city 填的是北京而不是香蕉"}}$$
...
$$\text{10 轮任务的成功率} = 0.97^{10} = \mathbf{74\%}$$ ❌

$$\boxed{\text{这就是为什么约束解码是必需的，不是可选的（第 14 章 Q8）}}$$
...
**关键认知（这是本题的题眼）**：
$$\boxed{\text{"停下来"是一个学出来的行为，不是硬编码的规则}}$$
...
$$\boxed{\text{这是协作式多任务（cooperative multitasking）—— 模型主动让出控制权}}$$
...
$$\boxed{\text{所以：把控制标记加进词表，是 Function Calling 的一个基础设施决策}}$$
...
$$\boxed{\text{LLM Agent = 协作式多任务；max\_tokens 是唯一的抢占式保底}}$$
...
**趋势**：
$$\boxed{\text{往"特殊 token"走 —— 更安全、更快、不会和正文冲突}}$$
...
| **① 该不该调** | **二分类** | 2 | 95%+ |
| **② 调哪个** | **N 分类** | ~20 | 90-95% |
| **③ 参数填什么** | ⭐ **开放生成** | **$\lvert V \rvert^{L}$（巨大！）** | **80-90%** ❌ |
| **④ 结果怎么用** | 阅读理解 | — | 90%+ |

$$\boxed{\text{①②④ 是选择题，③ 是填空题 —— 而填空题的搜索空间是指数级的}}$$
...
$$\boxed{\text{有 Prefix Caching 时，schema 长一点几乎免费 —— 别为了省 token 牺牲准确率}}$$
...
✅ search_product(keyword, category)
   ...
   → ★每个 2-3 个参数，简单明了★
```
**原则**：$$\boxed{\text{参数多 = 模型要做的决策多 = 错误率指数上升}}$$
...
$$\boxed{\text{注意：省的大头是 LLM 调用（2s），不是工具执行（0.5s）}}$$
...
$$\boxed{\text{LLMCompiler = 把"解释执行"（ReAct）变成"编译执行"（DAG）}}$$
...
$$\boxed{\text{并行调用和 Prefix Caching 是协同的：一个减轮次，一个减每轮的 prefill}}$$
...
$$\boxed{\text{关键：① 和 ⑤ 应该根本消灭，③ 的瞬时错误不该惊动模型 —— 只有 ② ④ 才该交给模型}}$$
...
$$\boxed{\text{★每一条错误信息，都是一次免费的 few-shot 教学★}}$$
...
$$\boxed{\text{判据不是"重不重要"，是"做错了能不能撤销"}}$$
...
$$\boxed{\text{这就是第 16 章的 RAG，只不过"语料"是工具的 description}}$$
...
**⚠️ 但有个<u>致命</u>的副作用**：
$$\boxed{\text{检索到的工具集随 query 变化 → schema 部分的前缀每次都不同 → ★Prefix Caching 全废★}}$$
...
```
$$\boxed{\text{把"连续的检索"变成"离散的路由" —— 用少量的缓存分片换命中率}}$$
- **5 个包 = 5 个缓存分片**，每个都能命中
...
$$\boxed{\text{"索引一个为检索优化的表示，返回一个为使用优化的表示"}}$$
...
$$\boxed{\text{MCP 让"工具太多"从"未来的问题"变成了"今天的问题"}}$$
...
**所以 MCP 的安全模型是**：
$$\boxed{\text{只装可信的 server + 协议层要求 human-in-the-loop}}$$
...
$$\boxed{\text{★把"读什么"的决策权留给应用/用户，是一道安全边界，不是设计上的偷懒★}}$$
...
**关键辨析（要能说清）**：
$$\boxed{\text{MCP 用 Function Calling，但 MCP 不是 Function Calling}}$$
...
```
$$\boxed{\text{单轮指标会系统性地高估真实的 Agent 能力}}$$
...
$$\boxed{\text{这能把"这一轮的能力"和"错误累积"解耦}}$$
...
| **选择题 vs 填空题** | ①②④ 是**选择题**，③ 是**填空题**（搜索空间 $\lvert V \rvert^L$）→ **`enum` 的本质是把填空变回选择** |
